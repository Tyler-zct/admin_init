  ...
  <div class="components-container">
    <el-form ref="form" :model="add" label-width="80px" v-loading="listLoading" element-loading-text="给我一点时间">
      <el-row :gutter="20" class="oneBill">
        ...
        <el-col :span=6>
          <label>{{$t('table.frequency')}}：</label>
          <span class="contColor">{{add.frequency}}</span>
        </el-col>
        <el-col :span=6>
      ...
      <el-row :gutter="20" class="oneBill">
        <el-col :span=9>
          ...
          <span class="contColor">{{add.originProvince}}-{{add.originCity}}-{{add.originDistrict}}</span>
        </el-col>
        <el-col :span=12>
      ...
      <el-row :gutter="20" class="oneBill">
        <el-col :span=20>
          <label>{{$t('table.requirement')}}：</label>
          <span class="contColor">{{add.requirement}}</span>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20" class="oneBill">
        ...
        <el-col :span=4>
          ...
          <span class="contColor">{{add.quantity}}</span>
        </el-col>
        <el-col :span=5>
        ...
        <el-col :span=4>
          <label>{{$t('table.released')}}：</label>
          <span class="contColor">{{add.released}}</span>
        </el-col>
      </el-row>

      <el-row :gutter="20" class="oneBill">
        <el-col :span=5>
          <label>{{$t('table.supply')}}：</label>
          <span class="contColor">{{add.supply}}</span>
        </el-col>
        <el-col :span=5>
      ...
      <el-row :gutter="20" class="oneBill">
        ...
        <el-col :span=7>
          ...
          <span class="contColor">{{add.deadline}}</span>
        </el-col>
        <el-col :span=7>
...
<script>
import { findPurchasing } from '@/api/api'
  ...
  methods: {
    getData() {
      ...
      findPurchasing(para).then((res) => {
        ...
        if (this.publisher.certification === true) {
          this.publisher.certification = '已认证'
        } else if (this.publisher.certification === false) {
          this.publisher.certification = '未认证'
        }
        if (this.publisher.corporate === true) {
          this.publisher.corporate = '已认证'
        } else if (this.publisher.corporate === false) {
          this.publisher.corporate = '未认证'
        }
        switch (this.publisher.level) {
          case 0:
            this.publisher.level = '普通用户'
            break
          case 1:
            this.publisher.level = '黄金加盟'
            break
          case 2:
            this.publisher.level = '铂金加盟'
            break
          case 3:
            this.publisher.level = '砖石加盟'
            break
          default: this.publisher.level = '未知'
            break
        }
        // console.log(this.publisher.level)
      })
    },
    handleSkip() {
