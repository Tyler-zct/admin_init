    ...
    <el-form ref="form" :model="add" label-width="80px" v-loading="listLoading" element-loading-text="给我一点时间">
      <el-row :gutter="20" class="oneBill">
        ...
        <el-col :span=6>
          ...
          <span class="contColor">{{add.name}}</span>
        </el-col>
      </el-row>
...
<script>
import { findRole, getMenus, getOperations, updateRole } from '@/api/api'
...
export default {
  ...
  methods: {
    getData() {
      ...
      findRole(para).then((res) => {
        this.listLoading = false
        this.add = res.data
        // console.log(this.add)
        const allSelect = res.data.menus
        const selectcontent = []
        const selectbill = []
        const selectuser = []
        const selectsystem = []
        ...
        // console.log(this.add.operations)
        _.forEach(this.add.operations, (value, key) => {
